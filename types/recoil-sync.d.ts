...
declare module 'refine' {
   ...
   */
  export function success<V>(value: V, warnings: ReadonlyArray<CheckFailure>): CheckSuccess<V>;
   ...
   * ```
   */
  export function assertion<T>(checker: Checker<T>, errorMessage?: string): AssertionFunction<T>;
   ...
   * values determined by a provided checker
   */
  export function dict<V>(valueChecker: Checker<V>): Checker<Readonly<{ [key: string]: V }>>;
   ...
   */
  export function writableDict<V>(valueChecker: Checker<V>): Checker<{ [key: string]: V }>;
   ...
   */
  export function jsonParserEnforced<T>(checker: Checker<T>, suffix?: string): JSONParser<T>;

  /**
   * convienience function to wrap a checker in a function
   * for easy JSON string parsing.
   */
  export function jsonParser<T>(checker: Checker<T>): JSONParser<T | undefined | null>;
   ...
   * ```
   */
  export function stringLiterals<T>(enumValues: { readonly [key: string]: T }): Checker<T>;
   ...
   * ```
   */
  export function asType<A, B>(checker: Checker<A>, cast: (input: A) => B): Checker<B>;

  /**
   * checker which asserts the value matches
   * at least one of the two provided checkers
   */
  export function or<A, B>(aChecker: Checker<A>, bChecker: Checker<B>): Checker<A | B>;
...
declare module 'recoil-sync' {
  ...
  import { DefaultValue, Loadable, AtomEffect } from 'recoil';
  import { Checker } from 'refine';
  ...
  export type ReadItem = (
    itemKey: ItemKey
  ) => DefaultValue | Promise<DefaultValue | unknown> | Loadable<DefaultValue | unknown> | unknown;
  ...
  export interface SyncEffectOptions<T> {
    ...
    write?: WriteAtom<T>;
    syncDefault?: boolean;
    actionOnFailure_UNSTABLE?: 'errorState' | 'defaultValue';
  }
  ...
  export type RecoilURLSyncJSONOptions = Omit<
    Omit<RecoilURLSyncOptions, 'serialize'>,
    'deserialize'
  >;
  ...
  export interface RecoilURLSyncTransitOptions
    extends Omit<Omit<RecoilURLSyncOptions, 'serialize'>, 'deserialize'> {
